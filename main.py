...
import random
import wandb
from models.custom import ChannelPruning
...
from yoloLoss import yoloLoss, detection_loss_4_yolo
...
#pre_weight = torch.load("ckpts/resnet18-test-best.pth",device)
#pretrained.load_state_dict(pre_weight)
#pretrained.to(device)
pretrained_loader = torchmodel.resnet18(weights='ResNet18_Weights.DEFAULT').to(device)
#pretrained_model = torchmodel.resnet18(pretrained=True)
#pretrained_model.to(device)

#pretrained_model = torchmodel.mobilenet_v2(pretrained=True).to(device)
#pretrained_model = torch.hub.load("chenyaofo/pytorch-cifar-models", "cifar10_mobilenetv2_x1_0", pretrained=True).to(device)
...
def main():
    ...
    new_state_dict = pretrained_loader.state_dict()
    dd = model.state_dict()
    ...
    if args.detection == True:
        train_root = '/home/harry09119/PyTorch-FBS-master/data/VOC2012/'
        #train_dataset = VOC(root=train_root,transform=composed,class_path='/home/harry09119/PyTorch-FBS-master/voc.names')
        #train_dataset = yoloDataset(root=file_root,list_file=['/home/harry09119/PyTorch-FBS-master/data/voc2012.txt'],train=True,transform = [transforms.ToTensor()] )
        #train_loader = DataLoader(train_dataset,batch_size=args.batch_size,shuffle=True,collate_fn=detection_collate)
        test_root = '/home/harry09119/PyTorch-FBS-master/data/VOC2007/' 
        test_dataset = VOC(root=test_root,transform=composed,class_path='/home/harry09119/PyTorch-FBS-master/voc.names' )
        val_loader = DataLoader(test_dataset,batch_size=args.batch_size,shuffle=False,collate_fn=detection_collate)
        criterion = yoloLoss(7,2,5,0.5)
        #soft_criterion = yoloLoss(7,2,5,0.5)
        soft_criterion = nn.SmoothL1Loss(reduction = "sum")
        train_dataset = VOC(root=train_root, transform=composed, class_path='/home/harry09119/PyTorch-FBS-master/voc.names')
        train_loader = torch.utils.data.DataLoader(dataset=train_dataset, batch_size=args.batch_size, shuffle=True, collate_fn=detection_collate)
    ...
    #rain_loader, val_loader = data_loader('.', dataset=args.dataset, batch_size=args.batch_size, workers=args.workers)
    ...
    if args.evaluate:
        ...
        for pruning_rate in args.test_pruning_rate:
            ...
            for m in model.modules():
                if isinstance(m, nn.BatchNorm2d):
                    mean_dict[pruning_rate].append(m.running_mean.abs().to(device).detach().numpy())
                    var_dict[pruning_rate].append(m.running_var.to(device).detach().numpy())
            validate(val_loader, model, criterion, args, args.epochs)
            for m in model.modules():
                if hasattr(m, 'rate'):
                    count_channel_dict[pruning_rate].append(np.array(m.count_channel))
...
def train(train_loader, model, criterion, soft_criterion, optimizer, epoch, args):
    ...
    loss_box_spot = AverageMeter('box_spot', ':.4e')
    loss_box_size = AverageMeter('box_size', ':.4e')
    loss_class = AverageMeter('class', ':.4e')
    loss_conf = AverageMeter('conf', ':.4e')
    loss_noconf = AverageMeter('noconf', ':.4e')

    top1 = AverageMeter('Acc@1', ':6.2f')
    top5 = AverageMeter('Acc@5', ':6.2f')
    progress = ProgressMeter(
        len(train_loader),
        [batch_time, losses, loss_box_spot, loss_box_size, loss_class, loss_conf, loss_noconf, top1, top5],
    ...
    total_loss = 0
    for i, (images, target, shape) in enumerate(train_loader):
        ...
        #print("Inputs!:",images.shape, target.shape)
        ...
        partition_points = [1.0, 2.0, 3.0, 4.0]

        #with torch.no_grad():
            #soft_logits = pretrained_model(images)
        ...
        if args.joint:
            ...
            #for pruning_rate in args.joint_pruning_rate:
                ...
                for partitions in partition_points:
                    for m in model.modules():
                        if hasattr(m, 'partition'):
                            m.partition = partitions
                            #print("[",pruning_rate,m.partition,"]")
                    ...
                    if pruning_rate == 0.0:
                        ...
                        #print(loss2.shape)
                        #loss1 = mixup_criterion(criterion,output_mix,targets_a, targets_b, lam)
                        #loss1 = 0
                        #loss1 = loss_func(criterion, output_mix)
                        #soft_target = torch.nn.functional.softmax(output, dim=1)
                        soft_logits = output
                        #loss2 = loss_func(criterion, output)
                        #loss = criterion(output, target)
                        #print(output.shape,target.shape)
                        #print(target)
                        #print("No.1!!!!!!!!")
                        loss, L_box_spot, L_box_size, L_class, L_confidence, L_noconfidence = detection_loss_4_yolo(output, target, device, prune=False)
                        #print(loss)
                        #print("[A] loss:",loss2)
                        #soft_target = torch.nn.functional.softmax(soft_logits, dim = 1)
                        #soft_target = loss2
                        #print("[0.0]:",soft_target.shape)
                        ...
                        #loss1 = 0
                        #loss1 = loss_func(criterion, output_mix)
                        #print(torch.mean(output),":", torch.mean(soft_logits))
                        #loss2 = criterion(output, soft_logits.detach())
                        #print("[B] loss2:", loss2)
                        #print('[not 0.0]:',loss2)
                        #print("No.2!!!!!!!!")
                        loss,\
                        ...
                        L_class,\
                        L_confidence,\
                        L_noconfidence = detection_loss_4_yolo(output, soft_logits.detach(),device, prune=True)
                        #print(loss)
                    ...
                    #total_loss = total_loss + 0.25 * loss
                    #acc1, acc5 = accuracy(output, target, topk=(1, 5))
                
                    #losses.update(loss.item(), images.size(0))
                    losses.update(loss.item(), images.size(0))
                    loss_box_spot.update(L_box_spot,images.size(0))
                    loss_box_size.update(L_box_size,images.size(0))
                    loss_class.update(L_class,images.size(0))
                    loss_conf.update(L_confidence,images.size(0))
                    loss_noconf.update(L_noconfidence,images.size(0))
            ...
            L_class,\
            L_confidence,\
            L_noconfidence = detection_loss_4_yolo(output, target,device, prune=False)
            loss2 = 0
            for m in model.modules():
                if hasattr(m, 'loss') and m.loss is not None:
                    loss2 += m.loss
...
def bn_calibration(train_loader, model, criterion, args):
    ...
    with torch.no_grad():
        ...
        #        target = target.cuda(args.gpu, non_blocking=True)
            ...
            L_class,\
            L_confidence,\
            L_noconfidence = detection_loss_4_yolo(output, target,device)
        ...
        if args.plot_bn is not None:
            for m in model.modules():
                if isinstance(m, nn.BatchNorm2d):
                    post_bn_mean.append(m.running_mean.to(device).detach().numpy())
                    post_bn_val.append(m.running.val.to(device).detach().numpy)
            """
...
class ResNetConv4(nn.Module): #extraction model class
    def __init__(self, model):
        ...
        self.pruning = ChannelPruning(256,256).to(device)
...
def validate(val_loader, model, criterion, args, epoch):
    ...
    with torch.no_grad():
        ...
        for i, (images, target, size) in enumerate(val_loader):
...
#            print("End send All\n")
#            ending = "end"
#            data_input = pickle.dumps(ending, protocol=pickle.HIGHEST_PROTOCOL)
...
#            print(end = start)
            ...
            loss,loss1,loss2,loss3,loss4,loss5 = detection_loss_4_yolo(final_output, target,device, prune=False)
            finish_time = timer()

            # measure accuracy and record loss
            #acc1, acc5 = accuracy(final_output, target, topk=(1, 5))
            losses.update(loss.item(), images.size(0))
            loss_box_spot.update(loss1,images.size(0))
            loss_box_size.update(loss2,images.size(0))
            loss_class.update(loss3,images.size(0))
            loss_conf.update(loss4,images.size(0))
            loss_noconf.update(loss5,images.size(0))
