...
class ChannelPruning(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.rate <= 0.75:
            ...
            #print(t.squeeze())
            #print(torch.nonzero(t.squeeze()))
            list = torch.nonzero(t.squeeze()).squeeze()
            #print(list.unsqueeze(1).shape)
            #package = torch.ones(x.shape[2]*x.shape[3],len(list))
            package = torch.index_select(x,1,list).view(len(list),-1)
            package = torch.cat((package,list.unsqueeze(1)),1)
            print(package.shape)
            #print(new_package.shape)
            print(package)

            unpack = package[:,-1].int()
            matrix = package[:,:-1].view(1,-1,112,112)
            print(unpack)
            print(matrix.view(1,-1,112,112))
            feature = torch.zeros(1,64,112,112)
            feature.index_add_(1,unpack,matrix)
            print(feature.shape,feature)
            #print(torch.index_select(x,1,list).shape)
            return mask
