...
'''
import math
from custom_cp import ChannelPruning
...
class VGG_mine(nn.Module):
    ...
    def forward(self, x):
        x = self.layer1(x)
        #print(x.shape)
        if self.pruning is not False and self.channel_prune1.rate !=0 :
            if self.partition==1.0:
                #print("1",self.channel_prune1.rate)
                x = x * self.channel_prune1(x)
        #print(x.shape)
        x = self.layer2(x)
        #print(x.shape)
        if self.pruning is not False and self.channel_prune2.rate !=0 :
            if self.partition==2.0:
                #print("2",self.channel_prune2.rate)
                x = x * self.channel_prune2(x)
        #print(x.shape)
        x = self.layer3(x)
        #print(x.shape)
        if self.pruning is not False and self.channel_prune3.rate !=0 :
            if self.partition==3.0:
                #print("3",self.channel_prune3.rate)
                x = x * self.channel_prune3(x)
        #print(x.shape)
        x = self.layer4(x)
        #print(x.shape)
        if self.pruning is not False and self.channel_prune4.rate !=0 :
            if self.partition==4.0:
                #print("4",self.channel_prune4.rate)
                x = x * self.channel_prune4(x)
        #print(x.shape)
        x = self.layer5(x)
        #print(x.size)
        x = x.view(-1,x.size(1))
